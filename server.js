...
const mqttClient = mqtt.connect({
    ...
    port               : parseInt(process.env.EMQX_PORT),
    protocol           : 'mqtts',        // TLS comme dans le firmware
    username           : process.env.EMQX_USERNAME,
    password           : process.env.EMQX_PASSWORD,
    clientId           : `vigi_server_${Date.now()}`,
    rejectUnauthorized : false,          // equivalent seclevel=0 du firmware
...
mqttClient.on('connect', () => {
    ...
    // Abonnement au topic de ta carte STM32
...
mqttClient.on('offline', () => {
    console.warn('[MQTT] ⚠️  Hors ligne, reconnexion automatique...');
});
...
app.listen(process.env.PORT, () => {
    console.log('\n══════════════════════════════════');
    console.log(`[API] ✅ Serveur demarre sur http://localhost:${process.env.PORT}`);
    console.log('[API] Endpoints disponibles:');
    console.log(`  GET http://localhost:${process.env.PORT}/api/status`);
    console.log(`  GET http://localhost:${process.env.PORT}/api/mesures`);
    console.log(`  GET http://localhost:${process.env.PORT}/api/mesures/derniere`);
    console.log(`  GET http://localhost:${process.env.PORT}/api/mesures/voies`);
    console.log(`  GET http://localhost:${process.env.PORT}/api/mesures/alarmes`);
    console.log('══════════════════════════════════\n');
});